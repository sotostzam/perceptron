...
def search(graph, app, origin, target):
    ...
    frontier.put((graph.evaluate(origin_node, target_node), (origin_node, [origin_node], 0)))
    while frontier:
        ...
        if neighbors:
            for neighbor_node, cost in neighbors:
                if neighbor_node.discovered is False:
                    ...
                    frontier.put((graph.evaluate(neighbor_node, target_node), (neighbor_node, new_path, new_cost)))
        else:
            return False
    return False
